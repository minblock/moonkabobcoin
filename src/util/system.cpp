...
const char * const BITCOIN_CONF_FILENAME = "moonkabobcoin.conf";
...
#else
    const char* pszModule = "moonkabobcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Moonkabobcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Moonkabobcoin";
#else
    // Unix
    return pathRet / ".moonkabobcoin";
#endif
#endif
}
