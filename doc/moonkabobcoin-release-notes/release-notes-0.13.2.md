Moonkabobcoin Core version 0.13.2 is now available from:

  <https://download.moonkabobcoin.org/moonkabobcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/moonkabobcoin-project/moonkabobcoin/issues>
...
updates will be released anymore. Without security updates, using a moonkabobcoin
...
ECDSA signatures inside Moonkabobcoin transactions now use validation using
...
Previous versions of Moonkabobcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Moonkabobcoin Core was to raise the default minimum
relay fee.

Moonkabobcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Moonkabobcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running moonkabobcoind without having to do any manual configuration.
...
Moonkabobcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Moonkabobcoin Core will relay transactions with
...
entirely in Moonkabobcoin Core 0.13, and it is at this time undecided whether the
...
Moonkabobcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Moonkabobcoin Core automatically creates a hidden service to listen on, without
manual configuration. Moonkabobcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Moonkabobcoin Core is listening, and
...
Moonkabobcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Moonkabobcoin
Core to use floating fees. By default, Moonkabobcoin Core will use floating
fees.
...
(default: `0.0002` MKB/kB).

At all times, Moonkabobcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MKB.
Furthermore, Moonkabobcoin Core will never create transactions paying less than
...
`moonkabobcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /moonkabobcoinrpc>
        ...
        # optional bypass moonkabobcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< moonkabobcoinrpc:<password>
    </Location>
...
Restarting moonkabobcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in moonkabobcoin.conf
...
moonkabobcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/moonkabobcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Moonkabobcoin Core code base has
...
- `moonkabobcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `moonkabobcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Moonkabobcoin scripting system that in combination with
...
Moonkabobcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Moonkabobcoin Core.
...
The two phases are distinct in the Moonkabobcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Moonkabobcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Moonkabobcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Moonkabobcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of moonkabobcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of moonkabobcoins being spent is exactly
...
- **Increased security for multisig:** Moonkabobcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Moonkabobcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Moonkabobcoin's current ledger (called the UTXO set),
  ...
  note that Moonkabobcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Moonkabobcoin users to individually opt-in to almost any change in the Moonkabobcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Moonkabobcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Moonkabobcoin
client to Moonkabobcoin Core 0.13.2 to signal for segwit support. When a
...
Since Moonkabobcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `moonkabobcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `moonkabobcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
